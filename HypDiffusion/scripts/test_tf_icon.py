...
def load_img(path, SCALE, pad=False, seg=False, target_size=None):
    if seg:
        ...
        image = padded_image
        resized_image = image.resize((224, 224), resample=PIL.Image.LANCZOS)
    else:
        image = Image.open(path).convert("RGB")
        w, h = image.size
    ...
    resized_image = resized_image[None].transpose(0, 3, 1, 2)
    resized_image = torch.from_numpy(resized_image)

    if pad or seg:
        return 2. * image - 1., 2. * resized_image - 1., new_w, new_h, padded_segmentation_map

    return 2. * image - 1., 2. * resized_image - 1., w, h
...
def load_model_and_get_prompt_embedding(model, opt, device, prompts, inv=False):
    ...
    if opt.scale != 1.0:
        if opt.mode == 'text':
            ...
            uc = model.get_learned_conditioning(opt.n_samples * [torch.zeros((1,3,224,224))])
    else:
        uc = None
    ...
    for subdir, _, files in os.walk(opt.root):
        for file in files:
            ...
            if file == files[-1]:
                ...
                init_image, resized_init_image, target_width, target_height = load_img(
                    opt.init_img, scale)
                ...
                ref_image, resized_ref_image, width, height, segmentation_map = load_img(
                    ...
                    with precision_scope("cuda"):
                        for prompts in data:
                            ...
                            if opt.mode == 'text':
                                ...
                                image = resized_init_image.clone().to(device)
                                image_2 = resized_ref_image.clone().to(device)
                                c_1, uc, inv_emb_1 = load_model_and_get_prompt_embedding(
                                    model, opt, device, image, inv=True)
                                c_2, _, inv_emb_2 = load_model_and_get_prompt_embedding(
                                    model, opt, device, image_2, inv=True)
                                c = (c_1 + c_2)/2
                                inv_emb = (inv_emb_1 + inv_emb_2)/2
                            ...
                            x_samples_bg = torch.clamp((x_samples_bg + 1.0) / 2.0, min=0.0, max=1.0)
