...
class FrozenDinoV2Encoder(AbstractEncoder):
    ...
    def __init__(self, device="cuda", freeze=True):
        ...
        if freeze:
            self.freeze()
        self.image_mean = torch.tensor([0.485, 0.456, 0.406]).unsqueeze(
            0).unsqueeze(-1).unsqueeze(-1)
        self.image_std = torch.tensor([0.229, 0.224, 0.225]).unsqueeze(
            0).unsqueeze(-1).unsqueeze(-1)
    ...
    def forward(self, image, inv=False):
        ...
        image = (image.to(self.device) - self.image_mean.to(self.device)
