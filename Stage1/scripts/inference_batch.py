...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help="evaluate at this precision",
        default="/home/mhf/dxl/Lingxiao/Codes/dreambooth/dreambooth_object"
    )
    parser.add_argument(
        "--object_depth_dir",
        type=str,
        help="evaluate at this precision",
        default="/home/mhf/dxl/Lingxiao/Codes/dreambooth/dreambooth_object_depth"
    )
    parser.add_argument(
        "--bg_dir",
        type=str,
        help="evaluate at this precision",
        default="/home/mhf/dxl/Lingxiao/Codes/dreambooth/bg_test"
    )
    parser.add_argument(
        "--bg_depth_dir",
        type=str,
        help="evaluate at this precision",
        default="/home/mhf/dxl/Lingxiao/Codes/dreambooth/bg_test_depth"
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        help="evaluate at this precision",
        default="/home/mhf/dxl/Lingxiao/Codes/dreambooth/dreambooth_output_pbe",
    )
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                image_names = os.listdir(opt.object_dir)
                for image_name in image_names:
                    if image_name.endswith('.jpg'):
                        ref_image_path = os.path.join(
                            opt.object_dir, image_name)
                        print("ref_image_path: ", ref_image_path)
                        background_names = os.listdir(opt.bg_dir)
                        for background_name in background_names:
                            if not background_name.endswith("mask.jpg"):
                                count = background_name.split('_')[-1].split('.')[0]
                                background_path = os.path.join(
                                    opt.bg_dir, background_name)
                                print("background_path: ", background_path)
                                filename = os.path.basename(background_path)
                                img_p = Image.open(
                                    background_path).convert("RGB").resize((512, 512))
                                image_tensor = get_tensor()(img_p)
                                image_tensor = image_tensor.unsqueeze(0)
                                ref_p = Image.open(ref_image_path).convert(
                                    "RGB").resize((224, 224))
                                ref_tensor=get_tensor_clip()(ref_p)
                                ref_tensor = ref_tensor.unsqueeze(0)
                                mask = Image.open(background_path.replace(
                                    '.jpg', '_mask.jpg')).convert("L").resize((512, 512))
                                ...
                                if not opt.skip_save:
                                    for i,x_sample in enumerate(x_checked_image_torch):
                                        ...
                                        if not os.path.exists(os.path.join(opt.save_dir, dir_name)):
                                            os.mkdir(os.path.join(opt.save_dir, dir_name))
                                        gen_path = os.path.join(opt.save_dir, dir_name, str(count)+'.jpg')
